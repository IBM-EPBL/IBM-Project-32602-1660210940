...
def alertmail():
    ...
    if request.method == 'POST' :
        ...
        var_list1.append(name) #0
        var_list1.append(mail) #1
        var_list1.append(contact) #2
        #var_list1.append(location)
        var_list1.append(bloodgroup) #3
        var_list1.append(unit) #4
        print(location)
...
def update():
   print(location)
   insert_sql = "UPDATE " + location + " SET UNITS = (UNITS - ?) WHERE BLOOD = ?"
   prep_stmt = ibm_db.prepare(conn, insert_sql)
   ibm_db.bind_param(prep_stmt, 1, var_list1[4])
   ibm_db.bind_param(prep_stmt, 2, var_list1[3])
   ibm_db.execute(prep_stmt)
   return redirect(url_for('home'))
